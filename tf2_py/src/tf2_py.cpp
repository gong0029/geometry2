...
static PyObject *pModulemw = NULL;
static PyObject *pModulemwtime = NULL;
...
{
  if (PyObject_HasAttrString(obj, "_sec") && PyObject_HasAttrString(obj, "_nanosec")) {
    PyObject *sec = PyObject_GetAttrString(obj, "_sec");
    PyObject *nanosec = PyObject_GetAttrString(obj, "_nanosec");
  ...
  if (PyObject_HasAttrString(obj, "_nanoseconds")) {
    PyObject *nanoseconds = PyObject_GetAttrString(obj, "_nanoseconds");
...
static int rosduration_converter(PyObject *obj, tf2::Duration *rt)
{
  if(PyFloat_Check(obj)) {
    auto fsec = std::chrono::duration<double>(PyFloat_AsDouble(obj)); 
    *rt = std::chrono::duration_cast<tf2::Duration>(fsec);
    return PyErr_Occurred() ? 0 : 1;
  }

  if(PyLong_Check(obj)) {
    *rt = std::chrono::duration_cast<tf2::Duration>(std::chrono::seconds(PyLong_AsLong(obj)));
    return PyErr_Occurred() ? 0 : 1;
  }
  PyErr_SetString(PyExc_TypeError, "duration must be float or int.");
  return 0;
  /*
  if (PyObject_HasAttrString(obj, "_sec") && PyObject_HasAttrString(obj, "_nanosec")) {
    PyObject *sec = PyObject_GetAttrString(obj, "_sec");
...
static PyObject *getLatestCommonTime(PyObject *self, PyObject *args)
{
  ...
  mw_time = PyObject_GetAttrString(pModulemwtime, "Time");
  if (!mw_time) {
    goto cleanup;
  }
  ...
  if (0 != PyDict_SetItemString(kwargs, "secs", seconds)) {
    goto cleanup;
  }
  if (0 != PyDict_SetItemString(kwargs, "nsecs", nanoseconds)) {
    goto cleanup;
  }

  ob = PyObject_Call(mw_time, call_args, kwargs);
...
bool staticInit() {
  ...
  pModulemw        = pythonImport("middleware");
  pModulemwtime = pythonImport("middleware.time");
  pModulebuiltininterfacesmsgs = pythonImport("builtin_interfaces.msg");
  pModulegeometrymsgs = pythonImport("geometry_msgs.msg");

  if (pModulemw == NULL)
  {
    printf("Cannot load mw module");
    return false;
  }

  if (pModulemwtime == NULL)
  {
    printf("Cannot load mw.time.Time module");
    return false;
  }
